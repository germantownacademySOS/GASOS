...
class BeaconInfo {
    ...
    let sound: String
    let pan: Float
    let immediateVolume: Float
    let farVolume: Float
    let nearRssiParameter: Float
...
//    let pan: Float /* panning. -1.0 is left, 0.0 is center, 1.0 is right. */
    ...
    init(name: String, uuid: UUID, majorValue: CLBeaconMajorValue, minorValue: CLBeaconMinorValue, sound: String, panning: Float,
         immediateVolume: Float, farVolume: Float, nearRssiParameter: Float, backgroundSound: String = "", backgroundVolume: Float = 0) {
        self.name = name
        self.uuid = uuid
        ...
        self.sound = sound
        self.pan = panning
        self.immediateVolume = immediateVolume
        self.farVolume = farVolume
        self.nearRssiParameter = nearRssiParameter
